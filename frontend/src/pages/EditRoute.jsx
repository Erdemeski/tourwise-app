...
const statusConfig = {
    draft: { label: 'Draft', variant: 'secondary', color: 'text-slate-500', bg: 'bg-slate-100 dark:bg-slate-800' },
    finished: { label: 'Finished', variant: 'default', color: 'text-blue-500', bg: 'bg-blue-100 dark:bg-blue-900/30' },
    shared: { label: 'Shared', variant: 'success', color: 'text-emerald-500', bg: 'bg-emerald-100 dark:bg-emerald-900/30' },
};
...
export default function EditRoute() {
    ...
    useEffect(() => {
        const fetchRoute = async () => {
            try {
                const res = await fetch(`/api/routes?routeId=${routeId}`, { credentials: 'include' });
                ...
                setItineraryId(route.itineraryId || null);
                setFormData({
                    title: route.title,
                    ...
                    season: route.season || 'all',
                    terrainTypes: (route.terrainTypes || []).join(', '),
                    allowForks: route.allowForks,
                    allowComments: route.allowComments,
                    overview: route.overview || '',
                    itinerary: route.itinerary || '',
                    highlights: route.highlights || '',
                    tips: route.tips || '',
                });
                setWaypoints(route.waypointList || []);
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok) {
                setSubmitError(data.message || 'Failed to update route');
                return;
            }
            setSubmitError(null);
            navigate(`/routes/${data.slug || data._id}`);
        } catch (error) {
    ...
    if (loadingRoute) {
        return (
            <div className="min-h-screen bg-slate-50 dark:bg-[rgb(22,26,29)] flex flex-col items-center justify-center">
                <Loader2 className="h-10 w-10 animate-spin text-violet-500 mb-4" />
                <p className="text-slate-500 dark:text-slate-400">Loading route...</p>
            </div>
        );
    }
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">Edit Route</h1>
                                <p className="text-sm text-slate-500 dark:text-slate-400 truncate max-w-[200px] sm:max-w-none">
                                    {formData.title || 'Untitled route'}
                                </p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <Badge variant={currentStatus.variant} className="hidden sm:flex">
                                {currentStatus.label}
                            </Badge>
                            {routeSlug && (
                                <Link to={`/routes/${routeSlug}`}>
                                    <Button variant="outline" size="sm" className="hidden sm:flex dark:border-gray-600">
                        ...
                        <Card className="border-slate-200 dark:border-gray-700 bg-white dark:bg-[rgb(32,38,43)] shadow-sm">
                            <CardContent className="pt-6">
                                <SectionHeader
                                    ...
                                    description="Control how your route appears to others"
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => {
                                                const newVisibility = value === 'finished' ? 'private' : formData.visibility;
                                                setFormData({ ...formData, status: value, visibility: newVisibility });
                                            }}
                                        >
                                        ...
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="text-sm text-violet-700 dark:text-violet-400 mt-1">
                                                    This route is linked to an AI itinerary. Edit stops and POI order there.
                                                </p>
                        ...
                        <Card className="border-slate-200 dark:border-gray-700 bg-white dark:bg-[rgb(32,38,43)] shadow-sm">
                            <CardContent className="pt-6">
                                <SectionHeader
                                    ...
                                    description="Update your route's name and description"
                                />
                        ...
                        <Card className="border-slate-200 dark:border-gray-700 bg-white dark:bg-[rgb(32,38,43)] shadow-sm">
                            <CardContent className="pt-6">
                                <SectionHeader
                                    icon={Image}
                                    title="Media Gallery"
                                    description="Add or update photos for your route"
                        ...
                        <Card className="border-slate-200 dark:border-gray-700 bg-white dark:bg-[rgb(32,38,43)] shadow-sm">
                            <CardContent className="pt-6">
                                <SectionHeader
                                    icon={Mountain}
                                    title="Route Details"
                                    description="Update distance, duration, and terrain info"
                        ...
                        <Card className="border-slate-200 dark:border-gray-700 bg-white dark:bg-[rgb(32,38,43)] shadow-sm">
                            <CardContent className="pt-6">
                                <SectionHeader
                                    icon={FileText}
                                    title="Route Story"
                                    description="Update your route's detailed content"
                        ...
                        <Card className="border-slate-200 dark:border-gray-700 bg-white dark:bg-[rgb(32,38,43)] shadow-sm">
                            <CardContent className="pt-6">
                                <SectionHeader
                                    icon={MapPin}
                                    title="Waypoints"
                                    description="Manage stops along your route"
